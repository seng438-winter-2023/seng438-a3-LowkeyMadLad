...
//	 This tests the result of the first index using the table values
	...
	// Tests what happens when the values in the data are null
	@Test
	public void testNullDataInMockery() {
		// Look at the top of the class for more information about this
		Mockery mockingContext = new Mockery();
		final KeyedValues kv = mockingContext.mock(KeyedValues.class);
		mockingContext.checking(new Expectations() {
		...
		try {
			KeyedValues result = DataUtilities.getCumulativePercentages(kv);
			double x = (double)result.getValue(1);
			System.out.print(x);
			test = true;
			assertFalse("If this is thrown, the invalidParameterException is not thrown",test);
		} catch (Exception e){
	...
	// This tests when the data itself is null. Should throw invalidParameterException
	@Test
	public void testNullDataInGeneral() {
...
//
	...
	public void test_CreateNumberArray_UBNumElements_NominalValues() {
		...
		Number expected[] = new Number[100000000];
		Arrays.fill(expected, 1.0);
		Number actual[] = DataUtilities.createNumberArray(data);

		assertArrayEquals("creating a Number array with 100000000 elements and nominal elements", expected, actual);
	}
	...
	public void testNullValueColumnTotal() {
		...
		assertEquals("Summing a column with null values.",
		        13.52, DataUtilities.calculateColumnTotal(threeByThree, 0), .000000001d);
	}
	...
	@Test
	public void testNegRowCountNullvallColumnTotal() {
		mockingContext.checking(new Expectations() {
            {
                oneOf(threeByThree).getRowCount();
                will(returnValue(-3));
		...
		assertEquals("Summing a column with negative row count and null values.",
		        10.02, DataUtilities.calculateColumnTotal(threeByThree, 0), .000000001d);
	}
	...
	@Test
	public void testTooBigRowTotal() {
		final Values2D hugeRow = hugeContext.mock(Values2D.class);
		hugeContext.checking(new Expectations() {
			{
				oneOf(hugeRow).getColumnCount();
		        will(returnValue(2));
		        oneOf(hugeRow).getValue(0, 0);
		        will(returnValue(Double.MAX_VALUE));
		        oneOf(hugeRow).getValue(0, 1);
		        will(returnValue(Double.MAX_VALUE));
			}
		...
		boolean caught = false;
		try {
			DataUtilities.calculateRowTotal(hugeRow, 0);
		}
		catch (Exception InvalidParameterException){
			caught = true;
		}
		assertTrue("Sum of max values in a row.", caught);
	}
	
	@Test
	public void testTooSmallRowTotal() {
		final Values2D smallRow = hugeContext.mock(Values2D.class);
		hugeContext.checking(new Expectations() {
			{
				oneOf(smallRow).getColumnCount();
		        will(returnValue(2));
		        oneOf(smallRow).getValue(0, 0);
		        will(returnValue(Double.MIN_VALUE));
		        oneOf(smallRow).getValue(0, 1);
		        will(returnValue(Double.MIN_VALUE));
			}
		...
		boolean caught = false;
		try {
			DataUtilities.calculateRowTotal(smallRow, 0);
		}
		catch (Exception InvalidParameterException){
			caught = true;
		}
		assertTrue("Sum of maximum negative values in a row.", caught);
	}
